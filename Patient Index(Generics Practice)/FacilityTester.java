...
public class FacilityTester {
	...
	public void testEmptyAddPatient() {
		CurrentPatient patient = new CurrentPatient("Jane", "Doe", uHID1, 1010101, date1);
		emptyFacility.addPatient(patient);
		ArrayList<CurrentPatient> patients = emptyFacility.getInactivePatients(date3);
	...
	public void testGetPhysicialListEmpty() {
		ArrayList<CurrentPatient> patients = new ArrayList<CurrentPatient>();
		assertEquals(patients, emptyFacility.getPhysicianList());
	}
	...
	@Test
	public void testVerySmallLookupNHIDNull() {
		assertNull(verySmallFacility.lookupByUHID(new UHealthID("MMMM-0000")));
		
	}
	
	@Test
	public void testVerySmallLookupByPhysicianCountNull() {
		ArrayList<CurrentPatient> patients = new ArrayList<CurrentPatient>();
		assertEquals(patients, verySmallFacility.lookupByPhysician(00000000));
	...
	public void testSetPhysician() {
		smallFacility.setPhysician(new UHealthID("MOON-3769"), 8888888);
		assertEquals(smallFacility.lookupByPhysician(8888888).get(0), smallFacility.lookupByUHID(new UHealthID("MOON-3769")));
	}
	
	//ADDED
	@Test 
	public void testSetLastVisit() {
		smallFacility.setLastVisit(new UHealthID("JHSD-7483"), date1);
		ArrayList<CurrentPatient> actual = smallFacility.getInactivePatients(new GregorianCalendar(2020, 0, 0));
